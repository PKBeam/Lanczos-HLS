...
// STREAM TODO
void ColWorkers::exec(hls::stream<byte_t> &input, kernel_t kern_vals[2*LANCZOS_A], num_t output[IN_WIDTH][ROW_WORKERS]){
	col_compute_loop:
    for(col_minor_counter_t i = 0; i < IN_WIDTH; i++){
		#pragma HLS PIPELINE
        output[i][out_idx] = compute(input_buffers[i], kern_vals);
...
// STREAM TODO
void ColWorkers::step_input(hls::stream<byte_t> &input){
	colWorkers_stepBuffers:
    for(col_minor_counter_t i = 0; i < IN_WIDTH; i++){
		#pragma HLS PIPELINE
    	byte_t new_val = in_idx >= IN_HEIGHT? (byte_t) 0 : input.read();
    	input_buffers[i].shift_left(new_val);
    	//shift_down<byte_t, 2*LANCZOS_A>(input_buffers[i], in_idx >= IN_HEIGHT? (byte_t) 0 : input[in_idx][i]);
    }
    in_idx++;
...
// STREAM TODO
void ColWorkers::initialize(hls::stream<byte_t> &input){
    // clear and initialize buffer with first few values of input
    out_idx = 0;
    curr_offset = offset;
    // Get first value of in_idx
